...
long long gen_f_pc()
{
    return ((((ex_mem_curr->icode) == (I_JMP)) & !(ex_mem_curr->takebranch)
      ...
      (mem_wb_curr->valm) : (((if_id_curr->icode) == (I_JMPQ)) & (
          (id_ex_next->srca) == (ex_mem_next->deste))) ? 
      (ex_mem_next->vale) : (((if_id_curr->icode) == (I_JMPQ)) & (
          (id_ex_next->srca) == (ex_mem_curr->destm))) ? 
      (mem_wb_next->valm) : (((if_id_curr->icode) == (I_JMPQ)) & (
          (id_ex_next->srca) == (ex_mem_curr->deste))) ? 
      (ex_mem_curr->vale) : (((if_id_curr->icode) == (I_JMPQ)) & (
          (id_ex_next->srca) == (mem_wb_curr->destm))) ? 
      (mem_wb_curr->valm) : (((if_id_curr->icode) == (I_JMPQ)) & (
          (id_ex_next->srca) == (mem_wb_curr->deste))) ? 
      (mem_wb_curr->vale) : (((if_id_curr->icode) == (I_JMPQ)) & !(
          (id_ex_next->srca) == (ex_mem_next->deste) || (id_ex_next->srca)
           == (ex_mem_curr->destm) || (id_ex_next->srca) == 
          (ex_mem_curr->deste) || (id_ex_next->srca) == 
          (mem_wb_curr->destm) || (id_ex_next->srca) == 
          (mem_wb_curr->deste))) ? (d_regvala) : (pc_curr->pc));
}
...
long long gen_instr_valid()
{
    return ((if_id_next->icode) == (I_NOP) || (if_id_next->icode) == 
      ...
      (I_PUSHQ) || (if_id_next->icode) == (I_POPQ) || (if_id_next->icode)
       == (I_IADDQ) || (if_id_next->icode) == (I_JMPQ));
}
...
long long gen_d_srcA()
{
    return (((if_id_curr->icode) == (I_RRMOVQ) || (if_id_curr->icode) == 
        (I_RMMOVQ) || (if_id_curr->icode) == (I_ALU) || (if_id_curr->icode)
         == (I_PUSHQ) || (if_id_curr->icode) == (I_JMPQ)) ? 
